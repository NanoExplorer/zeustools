...
def _dTAI_UTC(time, leapsecond_to_time, leapseconds=leapseconds):
    ...
    transition_times = [t.replace(tzinfo=timezone.utc) for t in transition_times]
...
def tai_to_utc(tai_time):
    ...
    :param datetime tai_time: Datetime object containing TAI time.
    """
...
def utc_to_tai(utc_time):
    ...
    :param datetime utc_time: Datetime object containing UTC time. If this 
        object does not have a timezone, it is treated as if it is in the UTC time
        zone, which is NOT how naive datetimes are usually treated. 
        Datetimes with timezones are treated appropriately.
    """
...
def gps_to_utc(gps_time):
    ...
    :param datetime gps_time: Datetime object containing GPS time.
    """
...
def run_tests(print_database=True):
    import doctest
    doctest.testmod()
